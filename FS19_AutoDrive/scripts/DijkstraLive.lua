function AutoDrive:dijkstraLiveLongLine(current_in, linked_in, target_id, vehicle)
...
function AutoDrive:dijkstraLive(start, target, vehicle)
    local distanceToAdd = 0
    local angle = 0
    local result = false
                ...
                if #wayPoints[shortest_id].out > 0 then
                    for i, linkedNodeId in pairs(wayPoints[shortest_id].out) do
                        ...
                        if wp ~= nil then
                            result = false
                            target_found = false
                            result, target_found = AutoDrive:dijkstraLiveLongLine(shortest_id, linkedNodeId, target, vehicle)
...
vehicle - Vehicle used to drive the path. Necessary to take restrictions into account.
...
function AutoDrive:dijkstraLiveShortestPath(start_id, target_id, vehicle)
    ...
    if vehicle ~= nil then
        print("AH: Trying to find route with vehicle", vehicle:getName())
    end

    local wayPoints = ADGraphManager:getWayPoints()
    local ret = false
    ret = AutoDrive:dijkstraLive(start_id, target_id, vehicle)
    if false == ret then
